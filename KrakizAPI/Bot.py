...
class Bot:
	def __init__(self, token):
		...
		self.clear = False
		self.ownerId = client.getData()["userId"]
	...
	def kickAllMember(self, guildId):
		...
		for i in range(utils.getMemberCount(self, guildId) - 1):
			if members[i] != self.ownerId:
				th = threading.Thread(target=self.kickMember, args=(guildId, members[i],))
				th.start()
	...
	def banAllMember(self, guildId):
		...
		for i in range(utils.getMemberCount(self, guildId) - 1):
			if members[i ] != self.ownerId:
				th = threading.Thread(target=self.banMember, args=(guildId, members[i],))
				th.start()
